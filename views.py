...
class Img():
  #Image object has two values;
  #self.img an Image object that represents an image,
  #self.img_np_array a numpy array (size = [n*n*3]) that contains the pixels of the image in RGB.
  ...
  #Function that rescale the size of the image.
...
class Video():
  #Video object has three values;
  #1 - self.video_path: video's path in google drive.
  #2 - self.video: download the video to usable format.
  #3 - self.video_length: video's length (total frames).
...
# @title Star
#An object that represnts the locationd of a star throughout the video. The locations are recived after YOLOv8 model analyzed the video.
class Star():
  #Star object contain the location of a single star throughout the video.
  def __init__(self):
  ...
  def is_full_orbit(self):
    #It does it by calculating the delta distances between the first star and the rest (each),
    #and if we see that the distance is closing on zero again (on the graph) we can know that the star has at least one full orbit.
    #In a nutshell, we need to find the minimum point of the function, in this point the sign of two distances changes from positive to negative.
    #The self.x variable represents the gap between two different locations, since if we take two close points the error of that the YOLOv8 model made rise significantly. - (It's dynamic for each orbit).
  ...
  #A function that creates a plot that contains the distance of the star from its first location over the video.
  #It helps visualize how to the star move throughout the video.
  ...
  #Function that shows the orbit of the star in the video.
  #for each frame the location [x,y] is marked on a black image that is the size of the video frame.
...
# @title Stars
#An object that represents several Star objects.
class Stars():
  #Stars object has one value that contains an array of Star objects.
  ...
  #A function that marks all of the locations of the stars in the video in different colors - each star with his own color.
...
# @title Orbit
#An object that is similar to Star object, but it represents only stars that have made a full orbit.
#This means that every Orbit object is a Star object, but not all Star object is Orbit object.
#This separation is necessary since a star that have made a full orbit is defenitly orbiting a black hole.
#That means that Star objects that aren't Orbit objects maybe don't orbit around a black hole.
class Orbit():
#Transform the locations array to ann array that contains the first one full orbit of locations
  #Orbit object contains 4 values;
  #1 - self.final_index_of_full_orbit: index of the location of the star that have made a full orbit.
  #2 - self.locations: contains the locations of one full orbit.
  #3 - self.distances: array of the distances from location X on the orbit from the first location throughout the video.
  #4 - self.delta_distances: array of the derivative of the distances over the frames of the video.
  #The function runs over the orbit of the star, and finds the first minimum point that represents the point when a star have made one full orbit.
  #The step variable represents the gap between two different locations, in order to find the absolute minimum point.
  def __init__(self, locations):
    #We need to find the smallest distance from point 0
    final_index_of_full_orbit = 0
    loc1 = locations[0] #location zero
    distances= []
    x_distances = []
    #Calculate the distances from point zero.
    ...
    final_delta_distances = [] #Derivative of the distances over the frames.
    min_distance = 99999999.0
    final_step = 0  #The step requires for minimize the YOLOv8 error.
  ...
  #Create a plot that contains the distance of every location from the first location.
  def show_distances(self):
  ...
  #create a plot that contains the derivative of the distances function.
...
#In order to calulate the location of the black hole I used ellipse formulas because every orbit in space around a single object is elliptical.
  
  #Calculate the length 'a' of the ellipse:
  ...
  #Calculate the 'b' of the ellipse:
  ...
  #Calculate the location [x,y] of the black hole using ellipse formulas.
  def get_the_locations_of_the_black_hole(self):
    ...
    black_hole_location = []  #The location of the black hole.
    max_distance = 0.0
    max_distance_index = 0
    last_location = self.locations[0]
    #We get two focal points and we need to check who is closest to the fastest speed location of the star, this focal point is the location of the black hole.
    for i in range(1, len(self.locations)):
  ...
  #The function shows the orbit of the star with the elliptical key points:
  #The furthest point of the ellipse that the length between them represents '2*a'.
  #The middel point of the ellipse.
  #The shortest location from the middle that the length is equal to 'b'.
...
# @title Orbits
#Object that contains several Orbit objects in a signle array.
class Orbits():
  #Orbits contain one value that represents an array of Orbit objects.
  ...
  #The fucntion calculate the average location of the black hole from all of the Orbit objects. The function marks every orbit in a different color and marks the location of the black hole. This is the final image.
  def show_orbits_and_black_hole(self, img, img_size):
    ...
    if(len(self.orbits_array) == 0):
      return None
    black_hole_array = []
    #For every Orbit object in the array self.orbits_array we calculate the location of the black hole and append it to a black hole locations array.
...
# @title Yolo Class
#An object that make the work with yolo simpler.
class yolo_class:
...
#BUild a custom model
  ...
  #Predict a video
  #Save = True means that the predicted video is saved.
  def predict_img_files(self, file_path):
    results = self.model(file_path, stream = True)
    return results

# @title Excel File
#An object that has several function that makes the work with excel file easier. This object was mainly used for creating a dataset for the second AI model.
class Excel_File():
  #Excel object has 2 values:
  #1 - self.file_path: the path for the excel file in Google Drive.
  #2 - self.dict_data: the data in the excel file in Dictionary format.
  def __init__(self, file_path):
    self.file_path = file_path
    self.dict_data = {}
    #Is there is a file with this file path that already exist. Prevent the loss of the data because creating a new excel file with the exact file path can erase the data on the file that already exist.
  ...
  #The data needs to be type array, and the data should be a two dimantions array, which each row represent different types of data (keys)
  #***Without new keys and their data. The function return the amount of added data.    
    ...
    #Check if there is identical data
      ...
      for key_index in range(len(data_names)):
        ...
        for arr_index in range(len(data_arr[key_index])):
          if(data_arr[key_index][arr_index] != None): #If the data_arr is empty in certain index, that means that the data inside it already exist in the file.
...
#The function recives the results from the YOLOv8 prediction and deciding for each star in the video it's next location in the next frame using the minimal length.
def get_boxes_to_stars_for_video(video_results, real_amount_of_stars_in_a_video):
  ...
  for frame in video_results:
    ...
    #In the rest of the frames we can determine the next star location by what is the closest location to it
...
#The purpose of this function is to check for a specific star location the closest star location to it in the next frame.
...
#The function recives type *stars* array, and claculate the alpha between two vectors on a non full orbit star, and returns the index of the curve and the alpha varient.
...
#The function calculates the total distance that the star made throughout the video.
def total_distance_in_star_orbit(star):
  star_locations = star.get_locations()
  total_distance = 0
  step = 5  #The gap between two locations in the locations array.
...
#Training the second model, not necessary for the running of the code. 
